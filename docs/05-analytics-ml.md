...
Monetary accounting is calculated at a canonical five-minute grain over the reconstructed intraday heat profile. Each completed-day EnB is allocated to five-minute intervals using the median normalized heat shape from the previous seven completed days; the live day uses the same causal profile and never uses future observations. For every interval, `delta_energy = EnB_interval - actual_interval` in `MJ/m²`; monetary delta is `delta_energy × growing_area_m² × heat_rate_CAD/MJ`. The ledger accumulates gross avoided heat cost `sum(max(delta, 0))`, gross excess heat cost `sum(max(-delta, 0))`, and the single stakeholder balance `gross avoided - gross excess`. The chart may downsample display points, but the ledger remains five-minute. The dashboard also provides a linear 1,000 m² scale reference so the research-compartment value is not confused with a commercial-greenhouse total. Rate, area, effective date and source remain visible; configured tariff values are scenarios until their evidence is reviewed.
