...
Monetary accounting separates gross avoided heat cost `sum(max(EnB - actual, 0))`, gross excess heat cost `sum(max(actual - EnB, 0))`, and net impact `gross avoided - gross excess`. Each is converted using the configured heat rate and 62.5 m² growing area. The dashboard also provides a linear 1,000 m² scale reference so the research-compartment value is not confused with a commercial-greenhouse total. Rate, area, effective date and source remain visible; configured tariff values are scenarios until their evidence is reviewed.
